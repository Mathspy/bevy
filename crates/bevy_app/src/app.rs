...
impl App {
    ...
    ///
    /// Returns whether a reload occured or not
    ///
    ...
    pub fn handle_reload(&mut self) -> bool {
            ...
            })
        {
            ...
            if !events.is_empty() {
                *self = (reloader.0)(core::mem::replace(self, App::empty()));
                reloaded = true;
...
/// A resource holding a function that takes in an [`App`] and returns a reloaded [`App`]
///
...
#[derive(Resource)]
pub struct AppReloader(pub Box<dyn Fn(App) -> App + Send + Sync>);
