...
use bevy_app::{App, IsInitialized, Last, Plugin};
...
impl<T: Event> Plugin for WinitPlugin<T> {
    ...
    fn build(&self, app: &mut App) {
        // If this is the second time we are being initialized, avoid re-creating the event loop.
        // This is a restriction of the current winit implementation, the event loop can only ever
        // be created once
        if app.world().get_resource::<IsInitialized>().is_none() {
            ...
            #[cfg(target_os = "android")]
            {
                ...
                event_loop_builder
                    .with_android_app(bevy_window::ANDROID_APP.get().expect(msg).clone());
            }
